...
struct CalibrationView: View {
    ...
    enum CalibState: CaseIterable {
        case neutral, smileLeft, smileRight, pucker, finished
    }
    ...
    var body: some View {
            ...
            VStack {
                VStack(spacing: 8) {
                    Text(instructionTitle)
                        ...
                        .accessibilityLabel("Current Instruction: \(instructionTitle)")
                ...
                ZStack {
                    Circle().stroke(.white.opacity(0.1), lineWidth: 20).frame(width: 220, height: 220)
                    Circle().trim(from: 0, to: progress)
                        .stroke(
                            AngularGradient(colors: [stateColor.opacity(0.5), stateColor], center: .center, startAngle: .degrees(-90), endAngle: .degrees(270)),
                    ...
                    VStack(spacing: 4) {
                        if state == .neutral {
                            Image(systemName: "face.dashed").font(.system(size: 50)).foregroundStyle(.white.opacity(0.8))
                        } else {
                            Text(feedbackText).font(.system(size: 56, weight: .bold, design: .rounded)).contentTransition(.numericText())
                            Text("INTENSITY").font(.caption.bold()).tracking(2).foregroundStyle(.white.opacity(0.5))
                        }
                    }
                    .foregroundStyle(.white)
                }
                ...
                .accessibilityValue("\(feedbackText) complete")
        ...
        .onReceive(Timer.publish(every: 0.05, on: .main, in: .common).autoconnect()) { _ in handleLogic() }
    }
    ...
    private func handleLogic() {
        ...
        if currentValue > threshold {
            if !isHolding { isHolding = true; lastChangeTime = Date() }
            if state != .neutral { currentMax = max(currentMax, currentValue) }
    ...
    private func completeStep() {
        ...
        case .smileLeft: faceManager.setCalibrationMax(for: .smileLeft, value: currentMax); state = .smileRight
        case .smileRight: faceManager.setCalibrationMax(for: .smileRight, value: currentMax); state = .pucker
        case .pucker: faceManager.setCalibrationMax(for: .pucker, value: currentMax); state = .finished
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) { onCalibrationComplete() }
        case .finished: break
        }
        currentMax = 0.0; progress = 0.0; isHolding = false; lastChangeTime = Date()
    }
